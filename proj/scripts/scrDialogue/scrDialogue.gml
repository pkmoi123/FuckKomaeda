...
with(dialogueBox) {
    ...
    myMessage = messageGiver.npcDia[index1, index2]; //Grab the message from the messageGiver
    ...
    if(array_length_2d(messageGiver.npcDia, index1) > 1)
        hasChoices = true;
    else
        hasChoices = false;
    ...
    if(string_height_ext(string_hash_to_newline(myMessage), 16, maxLength) > maxHeight) {
        textHeight = string_height_ext(string_hash_to_newline(messageGiver.npcDia[index1, index2]), 16, maxLength); //Get how high the message is going to be
        ...
        for(i = 0; i < amount; ++i) {
            ...
            myMessage[i] = string_copy(messageGiver.npcDia[index1, index2], startingAt, (string_length(messageGiver.npcDia[index1, index2])) / amount);
    ...
    //If it's not too long, then just take the message and set it for index 0 of myMessage
        ...
        myMessage[messageIndex] = messageGiver.npcDia[index1, index2];
...
with(nameBox){
	maxLength1 = sprite_width - 48;
	maxHeight1 = sprite_height - 48;//adjust number for asset
	font_dialogue();
	myMessage1 = messageGiver.npcName[0,0];
}